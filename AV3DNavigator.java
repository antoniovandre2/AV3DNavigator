 ...
 * 
 * Última atualização: 25-01-2025. Não considerando alterações em variáveis globais.
 */
			...
			public void keyPressed(KeyEvent ke)
				...
				if (ContadorFrames == FramesDeslocamento)
					...
					if (keyCode == KeyEvent.VK_COMMA)
						...
						if (Math.abs(Teta) < TetaMax - DeslocamentoAngular) {if (Math.abs(Phi) < PhiMax - DeslocamentoAngular) {if ((! (Math.abs(Teta + DeslocamentoAngular) >= AntonioVandre.MaximoValorReal)) || (! ((Math.signum(Math.cos(Phi0Rotacao)) >= 0) && (Math.floor(Math.abs(Phi0Rotacao) / (Math.PI / 2)) * Math.PI / 2 + Math.abs(Phi0Rotacao) % (Math.PI / 2) * Math.cos(Rotacao) - Rot * Math.sin(Rotacao)) >= AntonioVandre.MaximoValorReal)) || (! ((Math.signum(Math.cos(Phi0Rotacao)) < 0) && (Math.signum(Phi0Rotacao) * (Math.ceil(Math.abs(Phi0Rotacao) / (Math.PI / 2)) * Math.PI / 2 - (Math.PI / 2 - (Math.abs(Phi0Rotacao) % (Math.PI / 2))) * Math.cos(Rotacao) - Rot * Math.sin(Rotacao))) >= AntonioVandre.MaximoValorReal)) || (! (Math.abs(xRotacao + RaioRot * Math.cos(Phi + Math.PI) * Math.cos(Teta)) >= AntonioVandre.MaximoValorReal)) || (! (Math.abs(yRotacao - RaioRot * Math.cos(Phi + Math.PI) * Math.sin(Teta)) >= AntonioVandre.MaximoValorReal)) || (! (Math.abs(zRotacao - RaioRot * Math.sin(Phi + Math.PI)) >= AntonioVandre.MaximoValorReal))) {Teta += DeslocamentoAngular; if (Math.signum(Math.cos(Phi0Rotacao)) >= 0) {Phi = Math.floor(Math.abs(Phi0Rotacao) / (Math.PI / 2)) * Math.PI / 2 + Phi0Rotacao % (Math.PI / 2) * Math.cos(Rotacao) - Rot0Rotacao * Math.sin(Rotacao); Rot = Math.floor(Math.abs(Phi0Rotacao) / (Math.PI / 2)) * Math.PI / 2 + Phi0Rotacao % (Math.PI / 2) * Math.sin(Teta - Teta0Rotacao) * Math.sin(Phi0Rotacao) + Rot0Rotacao;} else {Phi = Math.signum(Phi0Rotacao) * (Math.ceil(Math.abs(Phi0Rotacao) / (Math.PI / 2)) * Math.PI / 2 - (Math.PI / 2 - (Math.abs(Phi0Rotacao) % (Math.PI / 2))) * Math.cos(Rotacao) - Rot0Rotacao * Math.sin(Rotacao)); Rot = (Math.ceil(Math.abs(Phi0Rotacao) / (Math.PI / 2)) * Math.PI / 2 - (Math.PI / 2 - (Math.abs(Phi0Rotacao) % (Math.PI / 2)))) * Math.sin(Teta - Teta0Rotacao) * Math.sin(Phi0Rotacao) + Rot0Rotacao;}; x = xRotacao + RaioRot * Math.cos(Phi + Math.PI) * Math.cos(Teta); y = yRotacao - RaioRot * Math.cos(Phi + Math.PI) * Math.sin(Teta); z = zRotacao - RaioRot * Math.sin(Phi + Math.PI); ContadorFrames = 0;} else VariavelLimiteAtingido();} else {Phi -= Math.signum(Phi) * DeslocamentoAngular; ContadorFrames = FramesDeslocamento; Phit = Phi; FlagPhiInferior = 1;}} else {Teta -= Math.signum(Teta) * DeslocamentoAngular; ContadorFrames = FramesDeslocamento; Tetat = Teta; FlagTetaInferior = 1;}
						}
