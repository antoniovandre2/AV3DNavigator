...
int main ()
    ...
    for (int i = 1; i < resolucao; i++)
        for (int j = 0; j < resolucao; j++)
            {
            if (1 != resolucao - 1) printf("%f,%f,%f;%f,%f,%f|", a / (i * comprimento / resolucao) * cos(j * 2 * M_PI / resolucao), i * comprimento / resolucao - comprimento / 2, a / (i * comprimento / resolucao) * sin(j * 2 * M_PI / resolucao), a / ((i + 1) * comprimento / resolucao) * cos(j * 2 * M_PI / resolucao), (i + 1) * comprimento / resolucao - comprimento / 2, a / ((i + 1) * comprimento / resolucao) * sin(j * 2 * M_PI / resolucao));
